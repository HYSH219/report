...
9. list_available_skills
   - 功能：读取项目目录下.agents/skills目录的所有一级子目录，提取每个目录下SKILL.md文件的YAML front matter中的name和description字段
   - 参数：无
   - 返回：包含技能列表的JSON对象，每个技能包含name和description字段
...
3. 每五次聊天后，请提取关键信息，按照5W规则（谁Who、做了什么事What、什么时候When（可选）、在何处Where（可选）、为什么要做这个事Why（可选））提取多条关键信息，并记录到用户的本地电脑 /Users/atfa/Desktop/实验报告/log.txt 文件中。
4. 当聊天历史超过5轮，或聊天上下文长度超过3k时，需要主动触发聊天记录总结，对前70%左右的内容进行压缩，保留最后30%左右的内容原文。
5. 每次用户提交输入时，自动调用list_available_skills工具读取可用技能列表。
"""
    ...
    try:
        while True:
            ...
            # 自动调用list_available_skills工具
            print("Executing tool: list_available_skills")
            tool_call = {"name": "list_available_skills", "arguments": {}}
